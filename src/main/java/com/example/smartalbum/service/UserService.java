...
public class UserService {
    ...
    public boolean resetPassword(String username, String password, String mail, String code){
        if (!userService.hasUserName(username)) {
            throw  new UserExistsException("用户名不存在!");
        }
        if (!userService.hasMail(mail, true)) {
            throw new EmailFormatException("邮件不存在!");
        }

        UserExample example = new UserExample();
        example.createCriteria().andMailEqualTo(mail).andUsernameEqualTo(username);
        if (userDataService.queryByExample(example).size()<=0){
            throw new UserExistsException("邮箱不合法!");
        }
        ...
        if (mailService.hasEmailCode(mail, code) && mailService.delEmailCode(mail)) {
            ...
            return userDataService.updateSelective(user, example)>0;
        }
        return false;
    }
